...
- 调研/政策/价格/攻略类：最多 2 个 browser_agent 子任务，不要把同一座城市的调研
  拆成「政策 / 行情 / 板块」三个几乎重复的检索。需要落盘时再加 1 个
  document_agent。这类任务总子任务数优先 ≤ 3。
...
- 调研类先给 browser_agent；落盘文档给 document_agent（未指定格式写 HTML，
  ...
  用户只要 Markdown 时，子任务禁止出现「创建 Word / officecli / .docx」。
  本地代码/脚本给 developer_agent；媒体整理给 multi_modal_agent。
