"""v2 workforce graph: LLM coordinator + Act-loop workers + synthesize."""
...
def compile_v2_workforce_graph(
    *,
    ...
    def route_after(state: dict[str, Any]) -> Any:
        ...
        if action == "rework":
            ready = ready_subtasks(subtasks)
        if not ready:
            if all(str(t.get("status")) in {"completed", "failed"} for t in subtasks):
                return "synthesize"
            return "synthesize"
        sends: list[Send] = []
        for t in ready:
            ...
            if assignee not in workers:
                assignee = "browser_agent" if "browser_agent" in workers else next(iter(workers))
            sends.append(
                Send(
                    assignee,
                    {
    ...
    route_map: dict[str, Any] = {name: name for name in workers}
