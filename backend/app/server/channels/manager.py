...
def compose_channel_reply(
    *,
    ...
    """Pick the user-facing Feishu reply from graph events."""
    if error:
        text = f"任务失败：{error}"
    else:
        text = _strip_think(summary) or _strip_think(streamed) or "任务已完成。"
...
class ChannelManager:
    ...
    def list_plugins(self) -> list[dict[str, Any]]:
        ...
        for meta in BUILTIN:
            ...
            has_env = bool(
                os.environ.get("LARK_APP_ID") and os.environ.get("LARK_APP_SECRET")
            ) if meta["plugin_id"] == "lark" else False
            ...
            out.append(
                {
                    ...
                    "bot_username": "lark" if meta["plugin_id"] == "lark" and has_token else None,
                    "is_extension": False,
                    "coming_soon": coming,
    ...
    def ingest(
        ...
        *,
        ...
        if user is None:
            ...
            self._schedule(self._send_text(chat_id, PAIRING_HINT.format(code=pairing["code"])))
        ...
        if skill is not None and not skill_usable_via_remote(skill):
            self._schedule(self._send_text(chat_id, REMOTE_DENIED_MSG))
    ...
    async def _run_and_reply(self, task_req: TaskRequest, chat_id: str) -> None:
    ...
    def approve_pairing(self, code: str) -> dict[str, Any]:
        ...
        chat_id = rec.get("chat_id") or ""
        self._schedule(self._send_text(chat_id, PAIRING_OK))
        return {"ok": True}
