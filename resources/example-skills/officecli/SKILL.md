...
| Skill id (load_skill / disk) | When to use |
|------|-------------|
| `officecli-docx` | Reports, letters, memos, proposals, generic documents |
| `officecli-word-form` | Fillable forms, contract/SOW fill slots, intake / HR / compliance checklists. NOT for plain prose reports (route those to `officecli-docx`) |
| `academic-paper` | Journal / conference / thesis (not bundled yet). NOT for business reports (route those to `officecli-docx`) |

### PowerPoint (.pptx)

| Skill id (load_skill / disk) | When to use |
|------|-------------|
| `officecli-pptx` | Generic decks: board reviews, sales decks, all-hands, product launches |
| `officecli-pitch-deck` | **Fundraising only** — seed / Series A-C / SAFE / convertible / strategic raise. NOT for sales / product / board decks (route those to `officecli-pptx`) |
| `morph-ppt` | Cinematic Morph-animated presentations (not bundled yet). NOT for static decks (route those to `officecli-pptx`) |
| `morph-ppt-3d` | 3D Morph (not bundled yet). NOT for 2D-only Morph |

### Excel (.xlsx)

| Skill id (load_skill / disk) | When to use |
|------|-------------|
| `officecli-xlsx` | Generic workbooks, formulas, pivots, trackers |
| `officecli-financial-model` | Financial models, scenarios, projections. NOT for general data analysis (route those to `officecli-xlsx`) |
| `officecli-data-dashboard` | CSV/tabular data → KPI / analytics / executive dashboards with charts and sparklines. NOT for raw data tracking (route those to `officecli-xlsx`) |

Example: a fundraising deck task → `load_skill` / preload `officecli-pitch-deck` → follow its rules.
